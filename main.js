...
function TasksView({tasks,projects,users,cu,reload,setData,onSetReminder,initialStage,initialPriority,initialAssignee,initialTaskId,onClearInitialTask,teams,activeTeam}){
  ...
  const _lastTaskCelebration=useRef({key:'',at:0});
  const triggerTaskCelebration=(taskTitle,taskProjectId,taskId='')=>{
    // Show task celebration instantly, but suppress duplicate popups from optimistic update + API echo/reload.
    const key=String(taskId||taskTitle||'Task')+'|'+String(taskProjectId||'');
    const now=Date.now();
    if(_lastTaskCelebration.current.key===key && now-Number(_lastTaskCelebration.current.at||0)<8000)return;
    _lastTaskCelebration.current={key,at:now};
    setCelebTask({title:taskTitle,projectId:taskProjectId});
    if(_celebTimeout.current)clearTimeout(_celebTimeout.current);
    _celebTimeout.current=setTimeout(()=>setCelebTask(null),3200);
  };
  useEffect(()=>{
    const h=e=>{const d=(e&&e.detail)||{};triggerTaskCelebration(d.title||'Task',d.project||'',d.id||'');};
  ...
  const quickStage=async(tid,stage)=>{
    ...
    if(stage==='completed'||stage==='production'){
      ...
      if(tk)triggerTaskCelebration(tk.title,tk.project,tid);
    }
    const before=safe(tasks).find(t=>t.id===tid);
    setData&&setData(prev=>({...prev,tasks:(prev.tasks||[]).map(t=>t.id===tid?{...t,...payload,_localTs:Date.now()}:t)}));
    api.put('/api/tasks/'+tid,payload).then(r=>{
      if(r&&r.error&&before){
        setData&&setData(prev=>({...prev,tasks:(prev.tasks||[]).map(t=>t.id===tid?before:t)}));
        try{window._pfToast&&window._pfToast('error','Task update failed',String(r.error||'Please retry'));}catch{}
      }else if(r&&r.id){
        setData&&setData(prev=>({...prev,tasks:(prev.tasks||[]).map(t=>t.id===tid?{...t,...r}:t)}));
      }
    }).catch(()=>{
      if(before)setData&&setData(prev=>({...prev,tasks:(prev.tasks||[]).map(t=>t.id===tid?before:t)}));
      try{window._pfToast&&window._pfToast('error','Task update failed','Please retry.');}catch{}
    });
  };
...
function BillingInvoicesView({cu}){
  ...
  const [summary,setSummary]=useState({invoice_count:0,paid_total:0,outstanding_total:0,overdue_count:0,currency:'INR'});
  ...
  const createInvoice=async()=>{
    ...
    }catch(e){setInvoices(prev=>prev.filter(x=>x.id!==temp.id));setErr('Could not create invoice');}
    setSaving(false);
  };
...
function App(){
  ...
  const routeToNotification=useCallback(async(n={})=>{
    ...
    // Ticket notifications must be resolved before task notifications. Older backend
    // payloads used task_assigned with entity_type=ticket, which made ticket clicks open Kanban.
    let ticketId=s(n.ticket_id||(entityType==='ticket'?rawEntityId:''));
    const ticketTypes=['ticket','ticket_assigned','ticket_updated','ticket_deleted','ticket.created','ticket.updated','ticket.deleted'];
    if(!ticketId&&ticketTypes.includes(type)) ticketId=rawEntityId;
    if(!ticketId){const tk=byName(tickets,['title','subject','name']); if(tk)ticketId=String(tk.id);}
    if(entityType==='ticket'||ticketId||ticketTypes.includes(type)||contentLow.includes('ticket')||content.startsWith('🎫')){
    ...
    if(!taskId&&entityType!=='ticket'&&['task_assigned','status_change','comment','deadline','reminder','task_updated','task.created','task.updated','task'].includes(type)) taskId=rawEntityId;
