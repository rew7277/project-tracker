...
function DirectMessages({cu,users,dmUnread,onDmRead,dmEnabled=true,initialUserId=null,onClearInitial,onlineUsers=new Set()}){
  ...
  const toggleRecording=async()=>{
    ...
    try{const stream=await navigator.mediaDevices.getUserMedia({audio:true});voiceChunksRef.current=[];const rec=new MediaRecorder(stream);mediaRecorderRef.current=rec;rec.ondataavailable=e=>{if(e.data&&e.data.size)voiceChunksRef.current.push(e.data);};rec.onstop=()=>{stream.getTracks().forEach(t=>t.stop());uploadVoiceBlob(new Blob(voiceChunksRef.current,{type:'audio/webm'}));};rec.start();setRecording(true);}catch(e){alert('Microphone permission is required to record voice notes.');}
  };
          ...
          <div key=${m.id} onClick=${ev=>{ev.stopPropagation();if(!m._pending&&!m._failed)setReactionPickerFor(v=>v===m.id?'':m.id);}} style=${{display:'flex',gap:8,alignItems:'flex-end',flexDirection:isMe?'row-reverse':'row',cursor:m._pending||m._failed?'default':'pointer'}}>
            ...
            <div style=${{display:'flex',flexDirection:'column',gap:2,alignItems:isMe?'flex-end':'flex-start',maxWidth:'68%'}}>
              <div style=${{position:'relative',display:'flex',flexDirection:'column',alignItems:isMe?'flex-end':'flex-start'}}>
                ...
                ${menuFor===m.id?html`<div onClick=${ev=>ev.stopPropagation()} style=${{position:'absolute',top:18,[isMe?'left':'right']:-8,display:'flex',flexDirection:'column',gap:2,padding:6,border:'1px solid var(--bd)',background:'var(--sf)',borderRadius:12,boxShadow:'0 15px 35px rgba(0,0,0,.35)',zIndex:30,minWidth:120}}>
                  <button class="btn" onClick=${()=>{setReplyTo(m);setMenuFor('');}}>↩ Reply</button>
                  ${isMe&&!m.deleted?html`<button class="btn" onClick=${()=>startEdit(m)}>✎ Edit</button>`:null}
                  <button class="btn" onClick=${()=>pinMessage(m)}>${m.pinned||m.pinned===1?'Unpin':'📌 Pin'}</button>
                  ${isMe&&!m.deleted?html`<button class="btn" onClick=${()=>deleteMessage(m)}>🗑 Delete</button>`:null}
                </div>`:null}
...
function TicketsView({cu,users,projects,onReload,activeTeam,initialAssignee,initialStatus,initialTicketId,onClearInitialTicket}){
  ...
  useEffect(()=>{
    ...
    if(t){
      ...
      try{history.replaceState(null,'',workspaceBasePath(cu)+'tickets');}catch(e){}
    }
