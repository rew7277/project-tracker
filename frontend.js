...
function DirectMessages({cu,users,dmUnread,onDmRead,dmEnabled=true,initialUserId=null,onClearInitial,onlineUsers=new Set()}){
  ...
  const startGoogleMeetCall=async()=>{
    ...
    try{
      ...
      const meetUrl=(data&&(data.launchUrl||data.meetUrl))||'https://meet.new';
      if(meetUrl){
        window.open(meetUrl,'_blank','noopener,noreferrer');
        loadMsgs(toId,'google-meet-launcher-opened');
        const hint=(data&&data.note)||'Opening Google Meet. Copy the generated Meet link and paste it in this DM.';
        if(typeof window.showToast==='function') window.showToast(hint,'success');
        else alert(hint);
      }else{
        alert('Unable to open Google Meet. Please try again.');
      }
    }catch(e){
