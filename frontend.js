        ...
        <div style=${{display:'flex',flexDirection:'column',gap:14,marginBottom:6}}>
          ${tab==='register'?html`
            ...
            <div>
              ...
              <input class="ap-inp" type="date" value=${birthDate} onInput=${e=>setBirthDate(e.target.value)}/>
...
function Sidebar({cu,view,setView,onLogout,unread,dmUnread,col,setCol,wsName,dark,setDark,teams,users,projects,tasks,teamCtx,setTeamCtx,activeTeam,wsDmEnabled=true,onlineUsers=new Set(),offlineMode=false}){
  ...
  const NAV_ICONS={
    ...
    billing: html`<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><rect x="3" y="4" width="18" height="16" rx="2"/><path d="M7 8h10"/><path d="M7 12h5"/><path d="M16 12h1"/><path d="M7 16h4"/><path d="M15 16h2"/></svg>`,
...
function Header({title,sub,dark,setDark,extra,cu,setCu,upcomingReminders,onViewReminders,notifs,onNotifClick,onMarkAllRead,onClearAll,activeTeam,teams,setTeamCtx}){
  ...
  const [profileBirthDate,setProfileBirthDate]=useState(cu&&cu.birth_date||'');
  const [profileBirthVisibility,setProfileBirthVisibility]=useState(cu&&cu.birth_date_visibility||'private');
  ...
  useEffect(()=>{
    setProfileBirthDate(cu&&cu.birth_date||'');
    setProfileBirthVisibility(cu&&cu.birth_date_visibility||'private');
  },[cu&&cu.birth_date,cu&&cu.birth_date_visibility]);
  return html`
            ...
            ${showProfile?html`
              <div style=${{position:'fixed',top:60,right:16,width:300,background:'var(--sf)',border:'1px solid var(--bd)',borderRadius:18,boxShadow:'0 8px 40px rgba(0,0,0,.18)',zIndex:9500,overflow:'hidden'}}>
                ...
                <div style=${{padding:'12px 14px',borderBottom:'1px solid var(--bd)'}}>
                  <div style=${{fontSize:11,fontWeight:800,color:'var(--tx)',marginBottom:8}}>Personal celebration</div>
                  <div style=${{display:'grid',gridTemplateColumns:'1fr 105px',gap:7}}>
                    <input class="inp" type="date" value=${profileBirthDate||''} onInput=${e=>setProfileBirthDate(e.target.value)} style=${{height:34,fontSize:12}}/>
                    <select class="inp" value=${profileBirthVisibility||'private'} onChange=${e=>setProfileBirthVisibility(e.target.value)} style=${{height:34,fontSize:11}}>
...
function OpsCommandCenter({cu,tasks,projects,users,tickets,notifs,onNav}){
  ...
  const panel=(title,body,sub)=>html`<div class="card" style=${{borderRadius:22,minHeight:0}}><div style=${{display:'flex',justifyContent:'space-between',alignItems:'center',gap:10,marginBottom:10}}><div style=${{fontSize:15,fontWeight:950,color:'var(--tx)'}}>${title}</div>${sub?html`<div style=${{fontSize:10,color:'var(--tx3)',fontWeight:800,textTransform:'uppercase',letterSpacing:.35}}>${sub}</div>`:null}</div>${body}</div>`;
  return html`
...
function renderChatContent(text){
  ...
  if(callIdMatch || /📞\s*Incoming video call/i.test(raw)){
    ...
    const serverTerminal=['ended','rejected','accepted','missed','cancelled','expired'];
    const localTerminal=['ended','rejected','missed','cancelled','expired'];
    // Never let stale localStorage resurrect Accept/Reject on completed history.
    // Server content/status is the source of truth once it is no longer ringing.
    let effectiveStatus=status && status!=='ringing' ? status : (stored||status);
    ...
    const isEnded=/accepted the call|rejected the call|ended the call|missed call/i.test(raw)||serverTerminal.includes(finalStatus)||localTerminal.includes(finalStatus);
...
function MessagesView({projects,users,cu,tasks}){
  ...
  const loadMsgs=useCallback(async(id,mode='switch')=>{
    ...
    if(cached){
      ...
      setMsgs([]);
      setLoadingChannel(id); // silent: prevents premature empty-state, no Opening channel UI
    }
  ...
  const sendChannelAttachmentFile=async(file)=>{
    if(!file||!pid)return;
    setAttaching(true);
    try{
      ...
      if(uploaded&&uploaded.id){
        await send(((file.type||'').startsWith('image/')?'🖼️ ':'📎 ')+uploaded.name+'\n/api/files/'+uploaded.id);
      }
    }catch(e){console.warn('[Channel] attachment upload failed',e);}
    finally{setAttaching(false);}
  };
  const handleChannelDrop=(ev)=>{ev.preventDefault();const file=ev.dataTransfer&&ev.dataTransfer.files&&ev.dataTransfer.files[0];sendChannelAttachmentFile(file);};
      ...
      <div style=${{display:'flex',gap:10,justifyContent:'center'}}>
        ...
        <div style=${{padding:'12px 16px',background:'var(--sf2)',borderBottom:'1px solid var(--bd)',flexShrink:0}}>
          ...
          <div style=${{display:'flex',alignItems:'center',gap:6}}>
            ...
            <div ref=${ref} onDragOver=${ev=>ev.preventDefault()} onDrop=${handleChannelDrop} style=${{flex:1,overflowY:'auto',padding:'13px 15px',display:'flex',flexDirection:'column',gap:0}}>
        ${(()=>{
          ...
          return groups.map((item,idx)=>{
            ...
            if(isSystem) return html`
              <div key=${m.client_msg_id||m.id} style=${{display:'flex',justifyContent:'center',padding:'3px 0',marginBottom:6}}>
            ...
            return html`
              <div key=${m.client_msg_id||m.id} style=${{display:'flex',gap:8,alignItems:'flex-end',flexDirection:isMe?'row-reverse':'row',marginBottom:6}}>
...
function DirectMessages({cu,users,dmUnread,onDmRead,dmEnabled=true,initialUserId=null,onClearInitial,onlineUsers=new Set()}){
  ...
  const normalizeDmList=useCallback((list)=>{
  ...
  const mergePendingReactionState=useCallback((peer,incoming)=>{
    ...
    const now=Date.now();
    const localList=threadCache.current.get(peer)||[];
    const localById=new Map(localList.map(m=>[m.id,m]));
    const merged=list.map(m=>{
      const local=localById.get(m.id);
      const until=pendingReactionUntil.current.get(m.id);
      if(until&&until>now&&local){
        return {...m,reactions:local.reactions||[],_reactionSyncing:true};
      }else if(until){
        pendingReactionUntil.current.delete(m.id);
      }
      if(local&&local._pending)return {...m,...local};
      return m;
    });
      ...
      if(msg&&msg.type==='call_status'&&data){
        ...
        if(data.status==='ringing'&&data.recipient===cu.id&&!dismissedCallIds.current.has(data.callId)){
          ...
          if(isFresh&&validMeet){
            ...
            setIncomingCall({callId:data.callId,from:sender.name||data.senderName||'Someone',meetUrl:data.meetUrl,peerId:data.sender});
          }
        }
        return;
      }
  ...
  const send=async()=>{
    ...
    const sendKey=recipient+'|'+c+'|'+(replyTo&&replyTo.id||'');
    if(dmRecentSendRef.current.key===sendKey && Date.now()-dmRecentSendRef.current.at<2500)return;
  ...
  const startGoogleMeetCall=async()=>{
    ...
    try{
      ...
      if(!r||!r.ok||!r.meetUrl){
        throw new Error((r&&r.error)||'Google Meet room was not created');
      }
      if(r.message){
        setMsgThreadId(recipient);
        setMsgs(prev=>{
          const exists=prev.some(x=>x.id===r.message.id);
          const next=exists?prev.map(x=>x.id===r.message.id?r.message:x):[...prev,r.message];
          threadCache.current.set(recipient,next);
          _saveDmCache&&_saveDmCache(recipient,next);
          return next;
        });
      }
  ...
  const toggleRecording=async()=>{
    ...
    try{
      ...
      rec.onstop=()=>{stream.getTracks().forEach(t=>t.stop());uploadVoiceBlob(new Blob(voiceChunksRef.current,{type:mime}));};
  ...
  const applyReactionMessage=useCallback((updated)=>{
    ...
    const until=pendingReactionUntil.current.get(updated.id);
    const patchList=list=>list.map(m=>{
      if(m.id!==updated.id)return m;
      // During the tiny server-confirm window, keep the optimistic reaction state stable.
  ...
  const toggleReaction=useCallback(async(messageId,emoji)=>{
    ...
    pendingReactionUntil.current.set(messageId,Date.now()+5000);
...
function ptWorkspacePrefixFromPath(){
  try{
    const VIEWS=['dashboard','ops','projects','tasks','messages','dm','tickets','timeline','reminders','settings','billing','team','productivity','ai-docs','timesheet','password-generator','vault','notifs'];
...
function ptRouteInfo(){
  try{
    const VALID=['dashboard','ops','projects','tasks','messages','dm','tickets','timeline','reminders','settings','billing','team','productivity','ai-docs','timesheet','password-generator','vault','notifs'];
    const seg=window.location.pathname.split('/').filter(Boolean);
...
function BillingInvoicesView({cu}){
  ...
  const [saving,setSaving]=useState(false);
  const [draft,setDraft]=useState({customer_name:'',customer_email:'',issue_date:new Date().toISOString().slice(0,10),due_date:'',status:'draft',notes:'',items:[{description:'Project management services',quantity:1,unit_price:0}]});
  ...
  const load=async()=>{setLoading(true);setErr('');const r=await api.get('/api/billing/invoices');if(r&& !r.error){setProfile(r.profile||{});setSummary(r.summary||{});setInvoices(r.invoices||[]);setNextNo(r.next_invoice_no||'');}else setErr((r&&r.error)||'Could not load billing');setLoading(false);};
  useEffect(()=>{load();},[]);
  const saveProfile=async()=>{setSaving(true);const r=await api.put('/api/billing/profile',profile);if(r&&!r.error){setProfile(r);await load();}else setErr((r&&r.error)||'Could not save billing profile');setSaving(false);};
  ...
  const createInvoice=async()=>{setSaving(true);setErr('');const r=await api.post('/api/billing/invoices',{...draft,currency:profile.currency||'INR',tax_rate:taxRate});if(r&&!r.error){setDraft({customer_name:'',customer_email:'',issue_date:new Date().toISOString().slice(0,10),due_date:'',status:'draft',notes:'',items:[{description:'Project management services',quantity:1,unit_price:0}]});setTab('invoices');await load();}else setErr((r&&r.error)||'Could not create invoice');setSaving(false);};
  const updateStatus=async(id,status)=>{await api.put('/api/billing/invoices/'+id+'/status',{status});load();};
  const card=(label,val,sub,icon)=>html`<div style=${{padding:18,borderRadius:20,background:'linear-gradient(180deg,var(--sf),rgba(255,255,255,.025))',border:'1px solid var(--bd2)',boxShadow:'0 18px 50px rgba(0,0,0,.18)'}}><div style=${{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:10}}><span style=${{fontSize:11,fontWeight:950,color:'var(--tx3)',textTransform:'uppercase',letterSpacing:.4}}>${label}</span><span style=${{fontSize:20}}>${icon}</span></div><div style=${{fontSize:25,fontWeight:950,color:'var(--tx)',lineHeight:1}}>${val}</div><div style=${{fontSize:11,color:'var(--tx3)',marginTop:8}}>${sub}</div></div>`;
  if(loading)return html`<div style=${{padding:24,color:'var(--tx2)'}}>Loading billing workspace…</div>`;
  if(err&&String(err).includes('admins'))return html`<div style=${{height:'100%',display:'grid',placeItems:'center',padding:24}}><div style=${{maxWidth:460,textAlign:'center',padding:26,border:'1px solid var(--bd)',borderRadius:22,background:'var(--sf)'}}><div style=${{fontSize:42}}>🔐</div><h2 style=${{fontSize:20,color:'var(--tx)'}}>Billing is restricted</h2><p style=${{color:'var(--tx2)',fontSize:13,lineHeight:1.6}}>Only workspace Admins, Owners, and Managers can manage invoice and billing information.</p></div></div>`;
  return html`<div style=${{height:'100%',overflow:'auto',padding:22,background:'var(--bg)'}}>
    <div style=${{display:'flex',justifyContent:'space-between',gap:14,alignItems:'flex-start',marginBottom:18}}>
      <div><div style=${{fontSize:28,fontWeight:950,color:'var(--tx)',letterSpacing:-.7}}>Billing & Invoices</div><div style=${{fontSize:13,color:'var(--tx2)',marginTop:5}}>Professional workspace billing center inspired by modern SaaS billing pages: overview, company invoice details, and downloadable-ready invoice history.</div></div>
      <button class="btn bp" onClick=${()=>setTab('create')}>+ Create invoice</button>
    </div>
    ${err?html`<div style=${{marginBottom:14,padding:12,borderRadius:14,background:'rgba(239,68,68,.10)',border:'1px solid rgba(239,68,68,.22)',color:'#fecaca',fontSize:12}}>${err}</div>`:null}
    ...
    ${tab==='overview'?html`<>
      <div style=${{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(210px,1fr))',gap:13,marginBottom:16}}>
        ${card('Total invoices',summary.invoice_count||0,'Generated in this workspace','🧾')}
        ...
        ${card('Next invoice no.',nextNo||'—','Auto-generated from prefix','🔢')}
      </div>
    ...
    ${tab==='invoices'?html`<div style=${{border:'1px solid var(--bd)',borderRadius:22,overflow:'hidden',background:'var(--sf)'}}>
      <div style=${{display:'grid',gridTemplateColumns:'150px 1fr 120px 120px 110px',gap:12,padding:'12px 14px',background:'var(--sf2)',borderBottom:'1px solid var(--bd)',fontSize:11,fontWeight:950,color:'var(--tx3)',textTransform:'uppercase'}}> <span>Invoice</span><span>Customer</span><span>Date</span><span>Total</span><span>Status</span></div>
      ${(invoices||[]).length?invoices.map(inv=>html`<div style=${{display:'grid',gridTemplateColumns:'150px 1fr 120px 120px 110px',gap:12,padding:'13px 14px',borderBottom:'1px solid var(--bd)',alignItems:'center'}}>
        <div style=${{fontWeight:900,color:'var(--tx)'}}>${inv.invoice_no}</div><div><div style=${{fontSize:13,color:'var(--tx)',fontWeight:800}}>${inv.customer_name||'Customer'}</div><div style=${{fontSize:11,color:'var(--tx3)'}}>${inv.customer_email||'No email'}</div></div><div style=${{fontSize:12,color:'var(--tx2)'}}>${inv.issue_date||'—'}</div><div style=${{fontWeight:900,color:'var(--tx)'}}>${money(inv.total,inv.currency)}</div><select class="inp" value=${inv.status} onChange=${e=>updateStatus(inv.id,e.target.value)} style=${{height:32,fontSize:11}}><option>draft</option><option>sent</option><option>paid</option><option>overdue</option><option>void</option></select>
      </div>`):html`<div style=${{padding:30,textAlign:'center',color:'var(--tx3)'}}>No invoices created yet.</div>`}
    </div>`:null}
    ${tab==='profile'?html`<div style=${{maxWidth:920,padding:18,borderRadius:22,background:'var(--sf)',border:'1px solid var(--bd)'}}>
      <div style=${{fontSize:16,fontWeight:900,color:'var(--tx)',marginBottom:14}}>Company invoice details</div>
      <div style=${{display:'grid',gridTemplateColumns:'repeat(2,minmax(0,1fr))',gap:12}}>
        ${[['legal_name','Legal company name'],['billing_email','Billing email'],['tax_id','GST / Tax ID'],['country','Country'],['address_line1','Address line 1'],['address_line2','Address line 2'],['city','City'],['state','State'],['postal_code','Postal code'],['currency','Currency'],['invoice_prefix','Invoice prefix'],['tax_rate','Tax rate %']].map(([k,l])=>html`<label style=${{fontSize:11,fontWeight:900,color:'var(--tx3)',textTransform:'uppercase'}}>${l}<input class="inp" value=${profile[k]||''} onInput=${e=>setProfile({...profile,[k]:e.target.value})} style=${{marginTop:6}}/></label>`)}
      </div>
      <label style=${{display:'block',marginTop:12,fontSize:11,fontWeight:900,color:'var(--tx3)',textTransform:'uppercase'}}>Default invoice notes<textarea class="inp" value=${profile.invoice_notes||''} onInput=${e=>setProfile({...profile,invoice_notes:e.target.value})} style=${{marginTop:6,minHeight:82}}></textarea></label>
      <button class="btn bp" disabled=${saving} onClick=${saveProfile} style=${{marginTop:14}}>${saving?'Saving…':'Save company details'}</button>
    </div>`:null}
    ${tab==='create'?html`<div style=${{maxWidth:980,padding:18,borderRadius:22,background:'var(--sf)',border:'1px solid var(--bd)'}}>
      ...
      <div style=${{display:'grid',gridTemplateColumns:'repeat(2,minmax(0,1fr))',gap:12,marginBottom:14}}>
        <input class="inp" placeholder="Customer / company name" value=${draft.customer_name} onInput=${e=>setDraft({...draft,customer_name:e.target.value})}/>
        <input class="inp" placeholder="Customer email" value=${draft.customer_email} onInput=${e=>setDraft({...draft,customer_email:e.target.value})}/>
        <input class="inp" type="date" value=${draft.issue_date} onInput=${e=>setDraft({...draft,issue_date:e.target.value})}/>
        <input class="inp" type="date" value=${draft.due_date} onInput=${e=>setDraft({...draft,due_date:e.target.value})}/>
      </div>
      <div style=${{border:'1px solid var(--bd)',borderRadius:16,overflow:'hidden'}}>
        <div style=${{display:'grid',gridTemplateColumns:'1fr 100px 140px 120px',gap:10,padding:10,background:'var(--sf2)',fontSize:11,fontWeight:900,color:'var(--tx3)',textTransform:'uppercase'}}><span>Description</span><span>Qty</span><span>Unit price</span><span>Amount</span></div>
        ${draft.items.map((it,i)=>html`<div style=${{display:'grid',gridTemplateColumns:'1fr 100px 140px 120px',gap:10,padding:10,borderTop:'1px solid var(--bd)'}}>
          <input class="inp" value=${it.description} onInput=${e=>setItem(i,'description',e.target.value)}/><input class="inp" type="number" value=${it.quantity} onInput=${e=>setItem(i,'quantity',e.target.value)}/><input class="inp" type="number" value=${it.unit_price} onInput=${e=>setItem(i,'unit_price',e.target.value)}/><div style=${{display:'flex',alignItems:'center',justifyContent:'flex-end',fontWeight:900,color:'var(--tx)'}}>${money(Number(it.quantity||0)*Number(it.unit_price||0))}</div>
        </div>`)}
      </div>
      <button class="btn bg" style=${{marginTop:10}} onClick=${()=>setDraft({...draft,items:[...draft.items,{description:'',quantity:1,unit_price:0}]})}>+ Add line item</button>
      ...
      <textarea class="inp" placeholder="Invoice notes" value=${draft.notes} onInput=${e=>setDraft({...draft,notes:e.target.value})} style=${{marginTop:14,minHeight:80}}></textarea>
      <button class="btn bp" disabled=${saving} onClick=${createInvoice} style=${{marginTop:14}}>${saving?'Creating…':'Create invoice'}</button>
    </div>`:null}
  </div>`;
}
...
function App(){
  ...
  useEffect(()=>{
    const h=async(ev)=>{
      ...
      if(action==='join'){
        const w=window.open(call.meetUrl,'_blank');
